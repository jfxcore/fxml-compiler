...
The object exists outside of the scene graph and is internally stored by the root node.
...
## Path to an object
The evaluation context may also be exposed with a field, getter, or `ObservableValue` on the [code-behind](../code-behind.html) class.
In this case, the value of `fx:context` is a [path](../markup-extension/expression/path.html).
