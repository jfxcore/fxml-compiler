...
The `format` parameter of a bidirectional binding expression specifies the [path](path.html) to a `java.text.Format` instance:
```xml
...
The `converter` parameter of a bidirectional binding expression specifies the [path](path.html) to a `javafx.util.StringConverter` instance:
```xml
