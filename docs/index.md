...
FXML/2 is a declarative markup language for JavaFX based on the
[FXML](https://openjfx.io/javadoc/24/javafx.fxml/javafx/fxml/doc-files/introduction_to_fxml.html)
markup language, adding powerful new features:

## Compile markup directly to bytecode
FXML/2 markup is compiled directly to bytecode, and doesn't require `FXMLLoader` to load the document at runtime.
...
Here's how an [FXML file](getting-started/standalone.html) is compiled to a Java class:
...
FXML/2 supports this with an optional [code-behind](code-behind.html) class to combine FXML markup and Java code.

## Embed markup into Java or Kotlin files

FXML markup can optionally be [embedded](getting-started/embedded.html) into Java or Kotlin source code,
...
All symbols referenced in FXML markup are resolved at compile time. Errors are surfaced early in the build,
...
FXML/2 can be extended with [markup extensions](markup-extension.html) for advanced markup processing scenarios.
...
FXML/2 does not use the markup/controller pattern as featured in classic FXML with `FXMLLoader`.
Instead, FXML/2 markup compiles down to scene graph nodes, optionally including a [code-behind](code-behind.html)
